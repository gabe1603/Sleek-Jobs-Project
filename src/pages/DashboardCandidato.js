        ...
        <Grid container justifyContent="center">
          <Grid item xs={12} md={10} lg={8}>
            <Card sx={{ p: 4, borderRadius: 5, boxShadow: "0 4px 24px #0001", bgcolor: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 3 }}>
              <Avatar src={user?.avatar} alt={user?.name} sx={{ width: 90, height: 90, mb: 2, border: '3px solid #6610f2' }} />
              ...
              <Button variant="outlined" startIcon={<EditIcon />} sx={{ borderRadius: 3, fontWeight: 600, textTransform: 'none', borderColor: '#6610f2', color: '#6610f2', '&:hover': { borderColor: '#6610f2', bgcolor: '#f3e8ff' }, width: '100%' }}>
                Edit Profile
              </Button>
            </Card>
            <Card sx={{ p: 3, borderRadius: 4, boxShadow: "0 4px 18px #0001", mb: 3 }}>
              <Typography variant="h6" sx={{ fontWeight: 700, mb: 2 }}>Vagas Inscritas</Typography>
              {applications.length === 0 ? (
                <Typography color="text.secondary">Nenhuma candidatura encontrada.</Typography>
              ) : (
                applications.map(app => (
                  <Box key={app.id} sx={{ mb: 2, p: 1, borderBottom: "1px solid #eee" }}>
            ...
            <Card sx={{ p: 3, borderRadius: 4, boxShadow: "0 4px 18px #0001", mb: 3 }}>
              <Typography variant="h6" sx={{ fontWeight: 700, mb: 2 }}>Currículo</Typography>
              {cv ? (
                <Box>
                  <Typography sx={{ fontWeight: 600, mb: 1 }}>Resumo Profissional:</Typography>
                  <Typography sx={{ whiteSpace: 'pre-line', mb: 2 }}>{cv.summary}</Typography>
                  {/* Adapte para exibir experiências, educação e idiomas se o backend retornar esses campos */}
                </Box>
              ) : (
                <Typography color="text.secondary">Nenhum currículo encontrado.</Typography>
              )}
            </Card>
          </Grid>
        </Grid>
